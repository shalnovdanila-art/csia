...
// ---------------- EMAIL (SendGrid API over HTTPS) ----------------
// Uses HTTPS (port 443), so it works on Render free tier (unlike SMTP ports).
// Required env vars:
//   SENDGRID_API_KEY = your SendGrid API key
//   MAIL_FROM        = your verified sender email (must match SendGrid verification)

const SENDGRID_API_KEY = process.env.SENDGRID_API_KEY;
const MAIL_FROM = process.env.MAIL_FROM;

const EMAIL_ENABLED = Boolean(SENDGRID_API_KEY && MAIL_FROM);

if (EMAIL_ENABLED) {
  sgMail.setApiKey(SENDGRID_API_KEY);
  console.log('✅ Email provider configured (SendGrid API)');
} else {
  console.warn('⚠️ Email is not configured (SENDGRID_API_KEY / MAIL_FROM missing). Menu emails will not be sent.');
}
...
async function sendMenuEmail(recipientEmail, menu) {
  if (!EMAIL_ENABLED) throw new Error('Email is not configured.');
  ...
  const msg = {
    to: recipientEmail,
    from: MAIL_FROM,   // must be a verified sender in SendGrid
    subject,
    text,
    // Optional HTML version (recommended if you want nicer formatting)
    // html: `<pre>${escapeHtml(text)}</pre>`
  };

  // SendGrid returns a promise; a successful call is commonly accepted with 202
